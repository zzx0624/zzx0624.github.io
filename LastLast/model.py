...
class Model(nn.Module):
    def __init__(self, d, tv, tq, to, layers=[64, 32, 16, 8]):
        ...
        # self.Wv = torch.nn.Parameter(torch.rand(64, tv))
        # self.Wq = torch.nn.Parameter(torch.rand(64, tq))
        self.Wo = torch.nn.Parameter(torch.rand(128, to))  # 2048-->310
        self.Tc = torch.nn.Parameter(torch.tensor(np.random.uniform(-1, 1, (tq, tv, to)),
                                                  dtype=torch.float, device="cuda", requires_grad=True))

        self.MLP_Embedding_User = torch.nn.Embedding(200, layers[0])
        self.Layer1 = torch.nn.Linear(192, layers[1])
    ...
    def forward(self, q_input, v_input, user_input):
        ...
        x_mm = x_mm.view(x_q.size(0), -1, self.Wo.size(1))
        x_mm = torch.bmm(x_q, x_mm)

        x_mm = x_mm.view(self.Wo.size(1), -1)
        x_mm = torch.mm(self.Wo, x_mm)
        item_inputs = x_mm.view(-1, self.Wo.size(0))
